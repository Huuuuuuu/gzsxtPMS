...
<body class="gray-bg">
				...
				<div class="ibox float-e-margins">					
					...
					<div class="ibox-content">
						<form action="user.action" method="POST">
							...
							<label>角色信息列表：</label>
							<input type="text" name="deptName" style="width:220px;height:30px;" value=""
							placeholder="请输入需要查询的组织名称"/>
							&nbsp;&nbsp;<button type="submit" style="height:30px;">查询</button>&nbsp;&nbsp;
							<button type="button" style="height:30px;float:right;">授权</button>
							<button type="button" id="deptRemove" style="height:30px;float:right;margin-right:20px;">删除</button>
							<button type="button" id="deptReplace" style="height:30px;float:right;margin-right:20px;">修改</button>							
							<a href="page/user/deptAdd.jsp" style="color:#6B6B6B;"><button type="button" style="height:30px;float:right;margin-right:20px;">添加</button></a>								
						</form>
					</div>					
					<div class="ibox-content" style="text-align:center;">						
						...
						<c:choose>
							...
							<c:otherwise>
								<a href="user.action?cmd=deptList&deptName=${requestScope.deptName}">首页</a>
								<a href="user.action?cmd=deptList&deptName=${requestScope.deptName}&pageCount=${requestScope.pageCount-1}&"style="margin-left:20px;">上一页</a> 
							</c:otherwise>
						</c:choose>	
						...
						<c:choose>
							...
							<c:otherwise>
								<a href="user.action?cmd=deptList&deptName=${requestScope.deptName}&pageCount=${requestScope.pageCount+1}"style="margin-left:20px;">下一页</a>
								<a href="user.action?cmd=deptList&deptName=${requestScope.deptName}&pageCount=${requestScope.totalPage}"style="margin-left:20px;">尾页</a>
							</c:otherwise>
						</c:choose>
	...
	<script src="js/user/deptList.js"></script>
	<script>
